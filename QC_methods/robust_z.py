...
class RobustZQC(QCMethod):
    ...
    def fit(self, train_df: pd.DataFrame) -> None:
        g = train_df.groupby(COL_NAME.TRADE)
    ...
    def score_day(self, day_df: pd.DataFrame) -> pd.Series:
        ...
        for idx, row in day_df.iterrows():
            ...
            zmax = float(np.nanmax(np.abs(z.values.astype(float)))) if len(z) else 0.0
            vals.append(np.clip(zmax / self.z_cap, 0.0, 1.0))
        s = pd.Series(vals, index=day_df.index, name="RobustZ_score")
        return s
